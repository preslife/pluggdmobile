...
for (const [label, route] of [
  ...
  assert.match(discover, new RegExp(`title: '${label}'[\\s\\S]*?route: '${route.replaceAll('/', '\\/')}'`), `${label} must remain a visual Discover gateway`);
}
assert.match(discover, /router\.push\('\/market'/, 'Creator Market must remain a Discover gateway');
...
assert.match(studio, /title: 'Connect Card'[\s\S]*?route: '\/studio\/connect-card'/, 'Studio must expose Connect Card management');
assert.match(studio, /route: '\/studio\/splits'/, 'Studio must expose split tools from Connect workflows');
