...
assert.match(accountMenu, /const previewCreator = __DEV__[\s\S]*const menuCreatorAccess = creatorAccess \|\| previewCreator/, 'Studio visual fixture must remain development-only and real creator access must remain the production authority');
assert.match(accountMenu, /if \(menuCreatorAccess\)[\s\S]*label: 'Studio'/, 'Studio account action must be creator-only outside the development visual fixture');
