...
assert.match(dockSource, /label:\s*'Discover'[\s\S]*route:\s*'\/discover'/, 'Discover must be the primary discovery tab');
assert.match(dockSource, /aliases:[\s\S]*'\/stage'/, 'Stage links must remain active aliases for Discover');
