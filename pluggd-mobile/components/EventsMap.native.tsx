...
function isValidCoordinate(point: EventMapPoint) {
...
function regionFor(points: EventMapPoint[]): Region {
  ...
  return {
    latitude: Math.min(90, Math.max(-90, (minLat + maxLat) / 2)),
    longitude: Math.min(180, Math.max(-180, (minLng + maxLng) / 2)),
    latitudeDelta: Math.min(80, Math.max((maxLat - minLat) * 1.7, 0.08)),
    longitudeDelta: Math.min(120, Math.max((maxLng - minLng) * 1.7, 0.08)),
  };
}
...
export function EventsMap({ points: sourcePoints, count, loading = false, onSelectEvent, onActiveEventChange, onPress }: Props) {
  ...
  useEffect(() => {
    ...
    const canFitSafely = region.latitudeDelta < 70 && region.longitudeDelta < 110;
  ...
  if (Platform.OS === 'android' && !androidMapsConfigured) {
    return (
      <View style={styles.emptyCard} accessible accessibilityLabel="Events map unavailable in this build">
        ...
        <Text style={styles.emptyCopy}>Map is unavailable here. Browse the event list instead.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.card}>
      <View style={[styles.mapWrap, compactLayout && styles.mapWrapCompact]}>
        <MapView
          ref={mapRef}
