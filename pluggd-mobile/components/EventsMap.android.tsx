...
import { MAPBOX_CONFIG, type EventMapPoint, type MapPoint } from '../src/lib/mapbox';
...
const MAPBOX_RUNTIME_TOKEN = MAPBOX_CONFIG.TOKEN.trim();

if (MAPBOX_RUNTIME_TOKEN) {
  void Mapbox.setAccessToken(MAPBOX_RUNTIME_TOKEN);
}
...
type CameraSettings =
  | { centerCoordinate: [number, number]; zoomLevel: number; bounds?: never }
  | {
...
function cameraFor(points: EventMapPoint[]): CameraSettings {
  if (!points.length) {
    return { centerCoordinate: [-0.1276, 51.5072], zoomLevel: 10 };
  }
  ...
  const hasArea = maxLat - minLat > 0.00001 || maxLng - minLng > 0.00001;
  if (!hasArea) {
    return { centerCoordinate: [longitudes[0], latitudes[0]], zoomLevel: 10.5 };
  }
  return { bounds: { ne: [maxLng, maxLat], sw: [minLng, minLat] } };
}
...
export function EventsMap({ points: sourcePoints, count, loading = false, onSelectEvent, onActiveEventChange, onPress }: Props) {
  ...
  const cameraSettings = useMemo(() => cameraFor(points), [points]);
  ...
  // Never instantiate the native SDK without its public runtime credential.
  // The separate secret download token is build-only and is never available
  // to this bundle.
  if (!MAPBOX_RUNTIME_TOKEN || mapLoadFailed) {
    return (
      <View style={styles.emptyCard} accessible accessibilityLabel="Events map temporarily unavailable">
        <View style={styles.emptyIcon}><MaterialIcons name="map" size={24} color="#ff6600" /></View>
        <Text style={styles.emptyTitle}>Map temporarily unavailable</Text>
        <Text style={styles.emptyCopy}>Browse the event list while mapping is offline.</Text>
      </View>
    );
  }
    ...
    <View style={styles.card}>
      <View style={styles.mapWrap}>
        <MapView
          ...
          scaleBarEnabled
          scaleBarPosition={{ bottom: 9, right: 9 }}
          scaleBarUnits="metric"
          attributionEnabled
          attributionPosition={{ bottom: 8, left: 8 }}
          logoEnabled
          logoPosition={{ bottom: 8, left: 38 }}
        ...
        >
          <Camera
            {...cameraSettings}
            padding={{ paddingTop: 52, paddingRight: 48, paddingBottom: 52, paddingLeft: 48 }}
