...
function useCultureShellStyles() {
  const theme = usePluggdTheme();
  return useMemo(() => StyleSheet.create({
    screen: { flex: 1, backgroundColor: theme.colors.background },
    loadingBlock: { minHeight: 120, alignItems: 'center', justifyContent: 'center', gap: 10 },
    loadingText: { color: theme.colors.textSecondary, fontSize: 13, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700' },
    emptyCard: { borderWidth: 1, borderColor: theme.colors.controlBorder, backgroundColor: theme.colors.surface, borderRadius: 14, padding: 16, gap: 8 },
    emptyIcon: { width: 44, height: 44, borderRadius: 12, backgroundColor: theme.colors.accentSoft, borderWidth: 1, borderColor: theme.colors.controlBorder, alignItems: 'center', justifyContent: 'center' },
    emptyTitle: { color: theme.colors.text, fontSize: 18, lineHeight: 23, fontFamily: pluggdFonts.displayBold },
    emptyBody: { color: theme.colors.textSecondary, fontSize: 13, lineHeight: 19, fontFamily: pluggdFonts.satoshiMedium, fontWeight: '600' },
    createActionList: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: theme.colors.controlBorder },
    createAction: { minHeight: 70, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: theme.colors.controlBorder, paddingHorizontal: 2, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 11 },
    createActionIndex: { width: 22, color: theme.colors.textMuted, fontFamily: pluggdFonts.satoshiBlack, fontSize: 9.5 },
    createActionIcon: { width: 44, height: 44, borderRadius: 5, backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.controlBorder, alignItems: 'center', justifyContent: 'center' },
    createActionCopy: { flex: 1, minWidth: 0, gap: 2 },
    createActionTitle: { color: theme.colors.text, fontSize: 14, lineHeight: 18, fontFamily: pluggdFonts.satoshiBold, fontWeight: '800' },
    createActionMeta: { color: theme.colors.textSecondary, fontSize: 12, lineHeight: 16, fontFamily: pluggdFonts.satoshiMedium, fontWeight: '600' },
  }), [theme]);
}
...
function ScreenFrame({
  ...
  const theme = usePluggdTheme();
  const shellStyles = useCultureShellStyles();
  const insets = useSafeAreaInsets();
  const tone = title === 'Live' ? 'live' : title === 'Backstage' || title === 'Community' ? 'community' : 'accent';

  return (
    <PremiumScreenBackdrop tone={tone} style={shellStyles.screen}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          ...
          {
            paddingTop: sharedHeader
              ? Math.max(insets.top + 78, 114)
              : Math.max(insets.top + 22, 58),
            paddingBottom: SCREEN_BOTTOM + insets.bottom,
          },
        ]}
      >
        <PremiumScreenHeader
          eyebrow={title === 'Create' ? 'Creator Studio' : 'PLUGGD'}
          title={title}
          subtitle={subtitle}
          tone={tone}
...
function LoadingState({ label = 'Loading PLUGGD...' }: { label?: string }) {
  ...
  const shellStyles = useCultureShellStyles();
  return (
    <View style={shellStyles.loadingBlock}>
      <ActivityIndicator color={theme.colors.accentText} />
      <Text style={shellStyles.loadingText}>{label}</Text>
    </View>
  );
}
  ...
  return (
    <View style={shellStyles.emptyCard}>
      <View style={shellStyles.emptyIcon}>
        <MaterialIcons name={icon} size={24} color={theme.colors.accentText} />
      </View>
      <Text style={shellStyles.emptyTitle}>{title}</Text>
      <Text style={shellStyles.emptyBody}>{body}</Text>
    </View>
  );
}
...
export function CreateHubScreen() {
  ...
  const actions = [
    ...
    { label: 'Upload Clip', meta: 'Short-form release clip', icon: 'movie-creation' as const, route: '/upload-clip' },
    { label: 'Thread', meta: 'Start a community thread', icon: 'forum' as const, route: '/create-post?type=thread' },
    { label: 'Event', meta: 'Manage event pulse', icon: 'confirmation-number' as const, route: '/creator/events' },
    { label: 'Creator Studio', meta: 'Open Creator Studio', icon: 'dashboard-customize' as const, route: '/studio' },
  ];

  return (
    <ScreenFrame sharedHeader title="Create" subtitle="Fast actions for posting, going live, and keeping your scene moving from mobile.">
      ...
      <View style={shellStyles.createActionList}>
        {actions.map((action, index) => (
          <Pressable accessibilityRole="button" key={action.label} style={shellStyles.createAction} onPress={() => router.push(action.route as any)}>
            <Text style={shellStyles.createActionIndex}>{String(index + 1).padStart(2, '0')}</Text>
            <View style={shellStyles.createActionIcon}><MaterialIcons name={action.icon} size={22} color={theme.colors.accentText} /></View>
            <View style={shellStyles.createActionCopy}>
              <Text style={shellStyles.createActionTitle}>{action.label}</Text>
              <Text style={shellStyles.createActionMeta}>{action.meta}</Text>
            </View>
            <MaterialIcons name="arrow-forward" size={19} color={theme.colors.textMuted} />
          </Pressable>
        ))}
      </View>
    </ScreenFrame>
...
const styles = StyleSheet.create({
  ...
  timelineHandle: {
    color: '#8E8E9F',
    fontFamily: pluggdFonts.satoshiRegular, fontWeight: '400',
  },
  timelineTime: {
    color: '#62627A',
    fontSize: 11,
  ...
  inlineTrackTitle: { color: TEXT, fontSize: 14, lineHeight: 18, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700' },
  inlineCreatorName: { color: '#8E8E9F', fontSize: 12, lineHeight: 15, fontFamily: pluggdFonts.satoshiRegular, fontWeight: '400' },
  ...
  inlineTime: { color: '#8E8E9F', fontSize: 10, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700' },
  ...
  inlineEventKicker: { color: EMERALD, fontSize: 11, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  inlineEventTitle: { color: TEXT, fontSize: 14, fontFamily: pluggdFonts.satoshiBold, fontWeight: '800' },
  timelineFooter: {
    marginTop: 14,
    marginLeft: 52,
    flexDirection: 'row',
  ...
  stageTopText: { color: TEXT, fontSize: 12, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  stageControlChain: {
    position: 'absolute',
    right: 16,
    top: '35%',
  ...
  stageControlLabel: { color: TEXT, fontSize: 12, lineHeight: 14, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700' },
  cartBadge: {
    width: 44,
    height: 44,
    borderRadius: 22,
  ...
  passKicker: { color: EMERALD, fontSize: 11, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  ...
  passStatValue: { color: TEXT, fontSize: 17, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  passStatLabel: { color: '#8E8E9F', fontSize: 11, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700', marginTop: 2 },
  ...
  frontRowText: { color: TEXT, fontSize: 10, fontFamily: pluggdFonts.satoshiBold, fontWeight: '800' },
  stageHook: { color: EMERALD, fontSize: 11, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900', textTransform: 'uppercase' },
  ...
  lowStockText: { color: EMERALD, fontSize: 10, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  ...
  artworkInitials: { color: '#FFFFFF', fontSize: 15, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
  fill: { width: '100%', height: '100%' },
  cardTitle: { color: '#FFFFFF', fontSize: 16, lineHeight: 20, fontFamily: pluggdFonts.satoshiBold, fontWeight: '800' },
  cardMeta: { color: '#B3B3B3', fontSize: 12.5, lineHeight: 17, fontFamily: pluggdFonts.satoshiMedium, fontWeight: '600' },
  ...
  postHandle: { color: '#737373', fontSize: 12, fontFamily: pluggdFonts.satoshiBold, fontWeight: '700', marginTop: 2 },
  postBody: { color: '#FFFFFF', fontSize: 15, lineHeight: 21, fontFamily: pluggdFonts.satoshiMedium, fontWeight: '600' },
  ...
  primaryActionText: { color: NOIR_DEEP, fontSize: 14, fontFamily: pluggdFonts.satoshiBlack, fontWeight: '900' },
