...
import { CARNIVAL_GUIDE_FALLBACK, CARNIVAL_STORY_FALLBACK, carnivalGuideStatus } from './carnivalGuideContent';
...
export async function loadCarnivalHub(): Promise<CarnivalHubBundle> {
  ...
  if (!data || ![1, 2].includes(data.schemaVersion)) throw new Error('The Carnival guide is being updated. Please try again shortly.');
  const remoteStories = Array.isArray(data.stories) ? data.stories : [];
  const fallbackSlugs = new Set(CARNIVAL_STORY_FALLBACK.map((story) => story.slug));
  const stories = [
    ...CARNIVAL_STORY_FALLBACK.map((fallback) => {
      const remote = remoteStories.find((story: { slug?: unknown }) => story?.slug === fallback.slug);
      return remote ? { ...fallback, ...remote } : fallback;
    }),
    ...remoteStories.filter((story: { slug?: unknown }) => typeof story?.slug === 'string' && !fallbackSlugs.has(story.slug)),
  ];
  return {
    ...(data as Omit<CarnivalHubBundle, 'guide'>),
    stories,
    guide: {
      ...CARNIVAL_GUIDE_FALLBACK,
      status: carnivalGuideStatus(),
    },
  } as CarnivalHubBundle;
}
