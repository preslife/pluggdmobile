...
const RESTRICTED: CommercePolicyDecision = {
  ...
  storefront: null,
  reason: 'Checkout is temporarily unavailable.',
  ...
  managementUrl: null,
  denialReason: 'Checkout is temporarily unavailable.',
  killSwitchState: {
    active: false,
    global: false,
    rule: false,
...
export async function resolveCommercePolicy(
  ...
  const requiresDigitalStorefront = request.kind !== 'physical_merch' && request.classification !== 'physical';
  if (requiresDigitalStorefront && billingAdapter && !storefront) {
