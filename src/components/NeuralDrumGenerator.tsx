...
// Lazy import Magenta Music to prevent AudioContext initialization on page load
let magentaMusic: typeof import('@magenta/music') | null = null;
const getMagentaMusic = async () => {
  if (!magentaMusic) {
    magentaMusic = await import('@magenta/music');
  }
  return magentaMusic;
};
...
export const NeuralDrumGenerator = ({ 
  ...
  const musicRNNRef = useRef<any | null>(null);

  const initializeMusicRNN = async () => {
    if (!musicRNNRef.current) {
      try {
        const mm = await getMagentaMusic();
  ...
  const convertPatternToNoteSequence = (pattern: PatternStep[][]): any => {
  ...
  const convertNoteSequenceToPattern = (noteSequence: any): PatternStep[][] => {
  ...
  const generatePattern = async () => {
    ...
    try {
      ...
      let seedSequence: any | undefined;
