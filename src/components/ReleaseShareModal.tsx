...
const ReleaseShareModal = ({ 
  ...
  genre,
  children 
}: ReleaseShareModalProps) => {
  const [embedWidth, setEmbedWidth] = useState(400);
  const [embedHeight, setEmbedHeight] = useState(200);
  const { shareToSocial, copyToClipboard, nativeShare } = useShare();

  const shareText = `Check out "${title}" by ${artist} - ${description}`;
  ...
  const generateEmbedCode = () => {
    return `<iframe src="${url}?embed=true" width="${embedWidth}" height="${embedHeight}" frameborder="0" allow="autoplay; encrypted-media"></iframe>`;
  };

  const handleCopyEmbed = () => {
    copyToClipboard(generateEmbedCode());
  };
            ...
            <div className="space-y-3">
              <div className="text-center p-4 bg-muted rounded-lg">
                ...
                <div className="flex items-center justify-center gap-2 mt-2">
                  {releaseType && <Badge variant="outline">{releaseType}</Badge>}
          ...
          <TabsContent value="embed" className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="p-3 bg-muted rounded border text-sm text-muted-foreground">
                ...
                Copy this code to embed the release player on your website
              </div>
            </div>
          </TabsContent>
        </Tabs>
