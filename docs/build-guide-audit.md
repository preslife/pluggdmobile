...
| Notification Preferences | Notification center shows events; database prefs now exist but UI toggles and emitter wiring are pending. | Expose account settings backed by `notification_prefs`, ensure emitters honour opt-outs, and document regression coverage. | 【F:BUILD GUIDES/POST_MVP_ROADMAP.md†L186-L195】【F:src/components/NotificationCenter.tsx†L1-L120】【F:supabase/migrations/20250930005000_notification_preferences.sql†L1-L30】 |
...
| Membership tiers & fan subs | Replace mocks with Supabase RPCs and Stripe sync; expose Discord sync panel.【F:BUILD GUIDES/POST_MVP_ROADMAP.md†L90-L123】 | Front-end calls `supabase.rpc('create_membership_tier')`, edge `membership-tier-stripe` exists.【F:src/hooks/useMembershipTiers.ts†L181-L337】【F:supabase/functions/membership-tier-stripe/index.ts†L1-L160】 | RPCs and sync queue now defined via dedicated migration; background job orchestration, error surfacing, and Discord tab remain outstanding.【F:supabase/migrations/20250930000000_membership_tier_queue_and_rpcs.sql†L6-L518】 |
...
1. ✅ Ship Supabase RPCs `create_membership_tier`, `update_membership_tier`, `delete_membership_tier` with role checks and Stripe sync triggers (post-commit call to `membership-tier-stripe`).【F:BUILD GUIDES/POST_MVP_ROADMAP.md†L90-L123】【F:src/hooks/useMembershipTiers.ts†L259-L337】【F:supabase/migrations/20250930000000_membership_tier_queue_and_rpcs.sql†L6-L518】
...
1. ✅ Provision notification preference table; wire account toggles and update the bell component to respect unread counts server-side instead of optimistic increments.【F:src/components/NotificationBell.tsx†L26-L200】【F:BUILD GUIDES/POST_MVP_ROADMAP.md†L188-L205】【F:supabase/migrations/20250930005000_notification_preferences.sql†L1-L59】
