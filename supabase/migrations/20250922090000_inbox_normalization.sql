...
-- Backfill legacy inbox data ------------------------------------------------------
DO $$
DECLARE
  v_has_legacy boolean;
  v_messages_populated boolean;
BEGIN
  v_has_legacy := to_regclass('public.inbox_messages_legacy') IS NOT NULL;
  IF v_has_legacy THEN
    SELECT EXISTS (SELECT 1 FROM public.inbox_messages) INTO v_messages_populated;

    IF NOT v_messages_populated THEN
      CREATE TEMP TABLE tmp_inbox_legacy_threads ON COMMIT DROP AS
      SELECT DISTINCT
        m.user_id,
        m.social_account_id,
        COALESCE(NULLIF(m.provider_thread_id, ''), m.id::text) AS legacy_thread_key,
        gen_random_uuid() AS thread_id
      FROM public.inbox_messages_legacy m;

      CREATE TEMP TABLE tmp_inbox_legacy_messages ON COMMIT DROP AS
      SELECT
        m.id,
        m.user_id,
        m.social_account_id,
        ...
        m.author_avatar_url,
        m.content,
        COALESCE(m.media_urls, '{}') AS media_urls,
        COALESCE(m.requires_response, false) AS requires_response,
        m.parent_post_id,
        m.parent_provider_post_id,
        m.permalink,
        ...
        COALESCE(m.is_spam, false) AS is_spam,
        m.moderation_status,
        m.created_at,
        m.received_at,
        t.thread_id,
        t.legacy_thread_key,
        COALESCE(m.received_at, m.created_at, now()) AS message_ts
      FROM public.inbox_messages_legacy m
      JOIN tmp_inbox_legacy_threads t
        ON t.user_id = m.user_id
       AND t.social_account_id = m.social_account_id
       AND t.legacy_thread_key = COALESCE(NULLIF(m.provider_thread_id, ''), m.id::text);
      ...
      WITH thread_bounds AS (
        SELECT
          ...
          social_account_id,
          legacy_thread_key,
          MIN(message_ts) AS created_at,
          MAX(message_ts) AS last_at,
          MAX(NULLIF(provider_thread_id, '')) AS provider_thread_id,
          COUNT(*) AS message_count
        FROM tmp_inbox_legacy_messages
        GROUP BY thread_id, user_id, social_account_id, legacy_thread_key
      ), thread_preview AS (
        SELECT DISTINCT ON (thread_id)
          thread_id,
          left(COALESCE(content, ''), 280) AS preview
        FROM tmp_inbox_legacy_messages
        ORDER BY thread_id, message_ts DESC
      )
      ...
      SELECT
        ...
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'legacy_thread_key', b.legacy_thread_key,
          'legacy_provider_thread_id', b.provider_thread_id,
          'legacy_message_count', b.message_count
        )),
        b.user_id,
        b.created_at,
        b.last_at,
        b.last_at,
        COALESCE(p.preview, '')
      FROM thread_bounds b
      LEFT JOIN thread_preview p ON p.thread_id = b.thread_id;
      ...
      SELECT
        ...
        m.user_id,
        'owner' AS role,
        MAX(CASE WHEN m.is_read THEN m.message_ts END) AS last_read_at
      FROM tmp_inbox_legacy_messages m
      ...
      SELECT DISTINCT
        ...
        'member' AS role
      FROM tmp_inbox_legacy_messages m
      WHERE m.social_account_id IS NOT NULL
      ON CONFLICT (thread_id, social_account_id) DO NOTHING;
      ...
      SELECT
        ...
        jsonb_strip_nulls(jsonb_build_object(
          'legacy_message', true,
          'legacy_user_id', m.user_id,
          'message_type', m.message_type,
          'parent_post_id', m.parent_post_id,
          'parent_provider_post_id', m.parent_provider_post_id,
