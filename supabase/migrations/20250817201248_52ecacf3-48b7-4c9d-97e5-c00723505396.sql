-- Phase 2: Battles System Database Schema
...
ALTER TABLE public.releases ADD COLUMN perk_access TEXT DEFAULT 'public' CHECK (perk_access IN ('public', 'subscribers', 'tier:pro', 'tier:premium'));
...
-- Add session room presence table
...
CREATE TABLE public.session_participants (
  ...
  user_id UUID NOT NULL,
  role TEXT NOT NULL DEFAULT 'viewer' CHECK (role IN ('host', 'collaborator', 'viewer')),
  joined_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  left_at TIMESTAMP WITH TIME ZONE
);

-- RLS for session rooms
ALTER TABLE public.session_rooms ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.session_participants ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Everyone can view session rooms" ON public.session_rooms FOR SELECT USING (true);
CREATE POLICY "Hosts can manage their rooms" ON public.session_rooms FOR ALL USING (auth.uid() = host_id);
CREATE POLICY "Users can create rooms" ON public.session_rooms FOR INSERT WITH CHECK (auth.uid() = host_id);

CREATE POLICY "Participants can view room participation" ON public.session_participants FOR SELECT USING (true);
CREATE POLICY "Users can join rooms" ON public.session_participants FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can leave rooms" ON public.session_participants FOR UPDATE USING (auth.uid() = user_id);

-- Create storage bucket for battle audio
INSERT INTO storage.buckets (id, name, public) VALUES ('battle-audio', 'battle-audio', true);
