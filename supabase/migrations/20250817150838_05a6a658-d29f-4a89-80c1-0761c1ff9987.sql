...
create policy parts_read on public.session_participants for select using (
  exists (select 1 from public.session_rooms r where r.id = session_participants.room_id and (r.host_id = auth.uid() or exists (
    select 1 from public.session_participants p where p.room_id = session_participants.room_id and p.user_id = auth.uid()
  )))
);

create policy parts_write on public.session_participants for all using (
  exists (select 1 from public.session_rooms r where r.id = session_participants.room_id and r.host_id = auth.uid())
) with check (
  exists (select 1 from public.session_rooms r where r.id = session_participants.room_id and r.host_id = auth.uid())
);
...
create policy files_access on public.session_files for all using (
  exists (select 1 from public.session_participants p where p.room_id = session_files.room_id and p.user_id = auth.uid())
  or exists (select 1 from public.session_rooms r where r.id = session_files.room_id and r.host_id = auth.uid())
) with check (
  exists (select 1 from public.session_participants p where p.room_id = session_files.room_id and p.user_id = auth.uid())
  or exists (select 1 from public.session_rooms r where r.id = session_files.room_id and r.host_id = auth.uid())
);
...
create policy notes_access on public.session_notes for all using (
  exists (select 1 from public.session_participants p where p.room_id = session_notes.room_id and p.user_id = auth.uid())
  or exists (select 1 from public.session_rooms r where r.id = session_notes.room_id and r.host_id = auth.uid())
) with check (true);

-- Session Feedback Policies
create policy feedback_access on public.session_feedback for all using (
  exists (select 1 from public.session_participants p where p.room_id = session_feedback.room_id and p.user_id = auth.uid())
  or exists (select 1 from public.session_rooms r where r.id = session_feedback.room_id and r.host_id = auth.uid())
) with check (true);
...
-- Session files storage policies
