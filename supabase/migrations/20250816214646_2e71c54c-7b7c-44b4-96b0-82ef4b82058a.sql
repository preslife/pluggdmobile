-- Phase 1 & 2: Fix Critical RLS Recursion Issues (Final)
...
-- Clear existing contests and add proper ones
DELETE FROM contests WHERE title LIKE '%2025%';
...
-- Add unique constraint to badge_definitions for badge_type if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'badge_definitions_badge_type_unique') THEN
        ALTER TABLE badge_definitions ADD CONSTRAINT badge_definitions_badge_type_unique UNIQUE (badge_type);
    END IF;
END $$;
