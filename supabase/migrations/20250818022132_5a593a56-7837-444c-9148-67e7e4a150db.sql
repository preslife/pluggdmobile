...
-- Update status enum to include dispute statuses
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payout_status_enum') THEN
        CREATE TYPE payout_status_enum AS ENUM ('pending', 'processing', 'paid', 'failed', 'disputed', 'on_hold', 'adjusted');
        ALTER TABLE public.producer_payouts ALTER COLUMN status TYPE payout_status_enum USING status::payout_status_enum;
    END IF;
END $$;
...
BEGIN
  ...
  IF total_percent > 100 THEN
    RAISE EXCEPTION 'Total split percentage cannot exceed 100%%. Current total would be: %%', total_percent;
  END IF;
