...
do $$
begin
  -- rooms: read all live/idle; write only host
  if not exists (select 1 from pg_policies where tablename='session_rooms' and policyname='rooms_read') then
    create policy rooms_read on public.session_rooms for select using (true);
  end if;
  if not exists (select 1 from pg_policies where tablename='session_rooms' and policyname='rooms_host_write') then
    create policy rooms_host_write on public.session_rooms for all
      using (auth.uid() = host_id)
      with check (auth.uid() = host_id);
  end if;

  -- participants: a user can see their room records; host can see all
  if not exists (select 1 from pg_policies where tablename='session_participants' and policyname='parts_read') then
  ...
  if not exists (select 1 from pg_policies where tablename='session_participants' and policyname='parts_write') then
  ...
  -- files/notes/feedback: visible to host + participants of the room
  if not exists (select 1 from pg_policies where tablename='session_files' and policyname='files_access') then
  ...
  if not exists (select 1 from pg_policies where tablename='session_notes' and policyname='notes_access') then
  ...
  if not exists (select 1 from pg_policies where tablename='session_feedback' and policyname='feedback_access') then
  ...
  -- social connections: user owns their connections
  if not exists (select 1 from pg_policies where tablename='social_connections' and policyname='conn_owner') then
    create policy conn_owner on public.social_connections for all
      using (auth.uid() = user_id) with check (auth.uid() = user_id);
  end if;

  -- unified inbox: user owns their inbox
  if not exists (select 1 from pg_policies where tablename='unified_inbox' and policyname='inbox_owner') then
    create policy inbox_owner on public.unified_inbox for all
      using (auth.uid() = user_id) with check (auth.uid() = user_id);
  end if;

  -- social posts: user owns their posts
  if not exists (select 1 from pg_policies where tablename='social_posts' and policyname='posts_owner') then
