...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_releases_status_approved_date 
ON public.releases(status, approved, release_date DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_releases_spotlight 
ON public.releases(spotlight) WHERE spotlight = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_beats_featured_price_date 
ON public.beats(is_featured, price_cents, created_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_fan_subscriptions_creator_status 
ON public.fan_subscriptions(creator_id, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_content_splits_type_id 
ON public.content_splits(content_type, content_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_producer_payouts_user_status_date 
ON public.producer_payouts(payee_user_id, status, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_community_posts_creator_date 
ON public.community_posts(creator_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_community_likes_post_user 
ON public.community_likes(post_id, user_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_community_comments_post_date 
ON public.community_comments(post_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_battle_votes_matchup_voter 
ON public.battle_votes(matchup_id, voter_user_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_tickets_event_user 
