...
create policy "Creators can manage their own release drafts"
...
create policy "Admins can view all release drafts"
  on public.release_drafts
  for select
  using (public.has_role(auth.uid(), 'admin'));

-- Update timestamp trigger
create trigger update_release_drafts_updated_at
...
create policy "Public can view release artwork"
  on storage.objects for select
  using (bucket_id = 'release-artwork');

create policy "Public can view release audio"
  on storage.objects for select
...
create policy "Users can upload their own release audio"
  on storage.objects for insert
  with check (bucket_id = 'release-audio' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Users can update their own release audio"
  on storage.objects for update
  using (bucket_id = 'release-audio' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "Users can delete their own release audio"
  on storage.objects for delete
  using (bucket_id = 'release-audio' and auth.uid()::text = (storage.foldername(name))[1]);