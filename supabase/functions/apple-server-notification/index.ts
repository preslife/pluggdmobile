...
import {
  verifyAppleNotificationEnvelope,
...
async function reconcilePlatformPlan(
  supabaseClient: SupabaseServiceClient,
  txInfo: TransactionInfo,
  plan: ApplePlatformPlan,
  notificationType: string,
  subtype: string | undefined,
  renewalInfo: RenewalInfo | null,
  _notificationAlreadyLogged: boolean,
) {
  const status = resolveStatus(notificationType, subtype);
  if (!status) {
    return { received: true, type: "platform_subscription", unchanged: true };
  }

  const originalTransactionId =
    txInfo.originalTransactionId ?? txInfo.transactionId;
  const { data: current, error: lookupError } = await supabaseClient
    .from("user_subscriptions")
    .select("id,user_id")
    .eq("apple_original_transaction_id", originalTransactionId)
    .maybeSingle();
  if (lookupError) {
    throw new Error(`PLUGGD plan lookup failed: ${lookupError.message}`);
  }

  const userId = current?.user_id ?? txInfo.appAccountToken;
  if (!userId) {
    console.warn(
      `[apple-notification] PLUGGD plan ${originalTransactionId} has no account mapping`,
    );
    return {
      received: true,
      type: "platform_subscription",
      warning: "No matching PLUGGD account",
    };
  }

  const periodEnd = txInfo.expiresDate
    ? new Date(txInfo.expiresDate).toISOString()
    : null;
  const purchaseDate = txInfo.purchaseDate
    ? new Date(txInfo.purchaseDate).toISOString()
    : null;
  ...
  const { data: loggedTransaction, error: loggedTransactionError } =
    await supabaseClient
      .from("iap_transactions")
      .select("id,user_id")
      .eq("transaction_id", txInfo.transactionId)
      .maybeSingle();
  if (loggedTransactionError) {
    throw new Error(`Apple transaction lookup failed: ${loggedTransactionError.message}`);
  }
  if (loggedTransaction && loggedTransaction.user_id !== userId) {
    throw new Error("Verified Apple transaction belongs to a different account");
  }
  if (!loggedTransaction) {
    const { error: transactionError } = await supabaseClient
      ...
      .insert({
        ...
        status: "validated",
        raw_receipt: null,
        idempotency_key: `apple-iap:${txInfo.transactionId}`,
      });
    if (transactionError && transactionError.code !== "23505") {
      throw new Error(`Failed to record verified Apple transaction: ${transactionError.message}`);
    }
  }

  const { data: entitlement, error: entitlementError } = await supabaseClient
    .rpc("platform_sync_apple_subscription", {
      p_user_id: userId,
      ...
      p_environment: txInfo.environment,
      p_auto_renew_status: renewalInfo
  ...
  if (entitlementError || !entitlement) {
    throw new Error(
      `PLUGGD plan reconciliation failed: ${
  ...
  console.log(
    `[apple-notification] PLUGGD plan user=${userId} tier=${entitlement.tier} status=${entitlement.status}`,
  );
  return {
    received: true,
    type: "platform_subscription",
...
serve(async (req) => {
  ...
  try {
    ...
    const verifiedNotification = await verifyAppleNotificationEnvelope(signedPayload);
    ...
    // The independently signed nested transaction is bound to the already
    // verified notification app identity, bundle and environment.
    if (!verifiedNotification.transaction) {
      throw new Error("Verified notification transaction was not decoded");
    }
    const txInfo = verifiedNotification.transaction as TransactionInfo;
    ...
    const allowlistedPlatformPlan = applePlatformPlan(productId);
    if (allowlistedPlatformPlan) {
      ...
      if (!mappedProduct) {
        return new Response(
          JSON.stringify({ received: true, warning: "Inactive platform product" }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
      const platformPlan: ApplePlatformPlan = {
      ...
      const result = await reconcilePlatformPlan(
        supabaseClient,
        txInfo,
        platformPlan,
        notificationType,
    ...
    // ── Resolve the verified creator-membership product and record ──
