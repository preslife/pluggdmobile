...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Revoking role ${role_id} from user ${discord_user_id} in guild ${guild_id}`)

    // Discord API call to revoke role
    const discordResponse = await fetch(
      `https://discord.com/api/v10/guilds/${guild_id}/members/${discord_user_id}/roles/${role_id}`,
      {
        ...
        headers: {
          ...
          'X-Audit-Log-Reason': 'Pluggd subscription role revoke'
        }
      }
    )

    if (discordResponse.ok || discordResponse.status === 204) {
      console.log(`Successfully revoked role ${role_id} from user ${discord_user_id}`)
    ...
    console.error('Role revoke error:', error)
    return new Response(JSON.stringify({ 
      success: false,
      error: error.message 
    }), {
