...
-- request_artist_link now targets existing schema: managed_profiles(label_id, profile_id, role)
...
begin
  ...
  if v_exists then
    select id into link_id from public.managed_profiles where label_id = p_label_id and profile_id = p_creator_profile_id;
    status := 'exists';
  else
    insert into public.managed_profiles(label_id, profile_id, role)
    values (p_label_id, p_creator_profile_id, 'distribution_only')
...
begin
  ...
  select mp.profile_id into v_profile_id from public.managed_profiles mp where mp.id = p_link_id;
  ...
  -- No status column in current schema; simply return the link as accepted
  link_id := p_link_id;
  status := 'accepted';
  return next;
end; $$;
